...
// Briansune 2023 Linux 5.4
	ether_addr_copy(pNetDev->dev_addr, pMacAddr);
	return 0;
}
...
// Briansune 2023 Linux 5.4
		ether_addr_copy(pNetDev->dev_addr, pDevOpHook->devAddr);
	...
	IN unsigned int AddrStep)
{
	...
	file_w = filp_open(fileName, O_WRONLY | O_CREAT, 0);
	if (IS_ERR(file_w)) {
		DBGPRINT(RT_DEBUG_TRACE, ("%s: Error %ld opening %s\n", __FUNCTION__,
			  -PTR_ERR(file_w), fileName));
	} else {
		if (file_w->f_op && file_w->f_op->write) {
			file_w->f_pos = 0;
			...
			sprintf(msg, "Dump all EEPROM values to %s\n",
				fileName);
		}
		filp_close(file_w, NULL);
	}
